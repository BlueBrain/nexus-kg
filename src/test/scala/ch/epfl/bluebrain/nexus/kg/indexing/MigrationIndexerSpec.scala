...
class MigrationIndexerSpec
  ...
  "A MigrationIndexer" should {
    "process 'instance created' events" in {
      val (id, schema, types, source, author) = (idCaptor, schemaCaptor, typesCaptor, sourceCaptor, authorCaptor)
      Mockito
        .when(repo.create(id.capture, schema.capture, types.capture, source.capture)(author.capture))
        .thenReturn(success)
    ...
    "process 'instance attachment created' events" in {
      val (id, rev, attr, author) = (idCaptor, revCaptor, attrCaptor, authorCaptor)
      Mockito
        .when(repo.unsafeAttach(id.capture, rev.capture, attr.capture)(author.capture))
        .thenReturn(success)
    ...
    "process 'instance attachment removed' events" in {
      ...
      val (id, id2, rev, fileName, author) = (idCaptor, idCaptor, revCaptor, stringCaptor, authorCaptor)
      Mockito
      ...
      Mockito
        .when(repo.unattach(id2.capture, rev.capture, fileName.capture)(author.capture))
        .thenReturn(success)
    ...
    "process 'instance updated' events" in {
      val (id, rev, types, source, author) = (idCaptor, revCaptor, typesCaptor, sourceCaptor, authorCaptor)
      Mockito
        .when(repo.update(id.capture, rev.capture, types.capture, source.capture)(author.capture))
        .thenReturn(success)
    ...
    "process 'instance deprecated' events" in {
      val (id, rev, author) = (idCaptor, revCaptor, authorCaptor)
      Mockito
        .when(repo.deprecate(id.capture, rev.capture)(author.capture))
        .thenReturn(success)
    ...
    "process 'context published' events" in {
      ...
      val (id, id2, rev, types, source, author) =
        (idCaptor, idCaptor, revCaptor, typesCaptor, sourceCaptor, authorCaptor)
      Mockito.when(repo.get(id.capture)).thenReturn(OptionT[Task, Resource](Task(Some(resource.copy(id = contextId)))))
      Mockito
        .when(repo.update(id2.capture, rev.capture, types.capture, source.capture)(author.capture))
        .thenReturn(success)
