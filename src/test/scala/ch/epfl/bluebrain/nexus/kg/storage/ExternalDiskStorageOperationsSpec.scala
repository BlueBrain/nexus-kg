...
import ch.epfl.bluebrain.nexus.kg.storage.Storage.ExternalDiskStorage
...
class ExternalDiskStorageOperationsSpec
    extends ActorSystemFixture("ExternalDiskStorageOperationsSpec")
    with WordSpecLike
    with Matchers
    with BeforeAndAfter
  ...
  sealed trait Ctx {
    ...
    // format: off
    val storage = ExternalDiskStorage(ProjectRef(genUUID), genIri, 1L, false, false, "SHA-256", endpoint, Some(cred.encrypt), genString(), Permission.unsafe(genString()), Permission.unsafe(genString()))
  ...
  "ExternalDiskStorageOperations" should {
    ...
    "verify when storage exists" in new Ctx {
      ...
      val verify = new ExternalDiskStorageOperations.Verify[IO](storage, client)
