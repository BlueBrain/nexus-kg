...
class FileAttributesSpec extends WordSpec with Matchers with TestHelper with Randomness with EitherValues {
  ...
  "A storage FileAttributes" should {
    ...
    "be converted to file attributes case class correctly" in new Ctx {
      ...
      FileAttributes(id, json).right.value shouldEqual expected
    }
    ...
    "reject when algorithm digest is missing" in new Ctx {
      ...
      FileAttributes(id, json).left.value shouldEqual
