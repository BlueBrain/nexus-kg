    ...
    "list storages" in new Context {
        ...
        responseAs[Json].removeKeys("@context") shouldEqual expected.deepMerge(
          Json.obj(
            "_next" -> Json.fromString(
              s"http://example.com/v1/storages/$organization/$project?deprecated=false&searchAfter=%5B%22two%22%5D"
            )
          ))
      }
      ...
      Get(s"/v1/resources/$organization/$project/storage?deprecated=false") ~> addCredentials(oauthToken) ~> Accept(
        ...
        responseAs[Json].removeKeys("@context") shouldEqual expected.deepMerge(
          Json.obj(
            "_next" -> Json.fromString(
              s"http://example.com/v1/resources/$organization/$project/storage?deprecated=false&searchAfter=%5B%22two%22%5D"
            )
          ))
      }
    }

    "list storages with searchAfter" in new Context {
      ...
      Get(s"/v1/storages/$organization/$project?deprecated=false&searchAfter=%5B%22one%22%5D") ~> addCredentials(
      ...
      Get(s"/v1/resources/$organization/$project/storage?deprecated=false&searchAfter=%5B%22one%22%5D") ~> addCredentials(
        oauthToken) ~> Accept(MediaRanges.`*/*`) ~> routes ~> check {
        status shouldEqual StatusCodes.OK
        responseAs[Json].removeKeys("@context") shouldEqual expected.deepMerge(
          Json.obj(
