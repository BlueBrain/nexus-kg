...
object Tag {
  ...
  final def apply(resId: ResId, json: Json): Either[Rejection, Tag] =
    // format: off
    for {
      graph   <- (json deepMerge tagCtx).asGraph(_.rootNode.toRight(rootNotFound()))
      ...
      tag     <- cursor.downField(nxv.tag).focus.as[String].left.map(_ => InvalidResourceFormat(resId.ref, "'tag' field does not have the right format."))
    } yield Tag(rev, tag)
  // format: on
