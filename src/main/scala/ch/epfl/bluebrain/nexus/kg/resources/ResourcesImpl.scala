...
import ch.epfl.bluebrain.nexus.kg.resources.ResourcesImpl.SchemaContext
...
/**
  * Resource operations.
  */
class ResourcesImpl[F[_]](implicit F: Monad[F],
                          repo: Repo[F],
                          projects: Projects[F],
                          elastic: ElasticClient[F],
                          esConfig: ElasticConfig)
    extends Resources[F] { self =>
  ...
  def list(project: ProjectRef, deprecated: Option[Boolean], pagination: Pagination)(
    ...
    projects.views(project).flatMap { views =>
      ...
      elasticView match {
        case Some(view) =>
          elastic.search(QueryBuilder.queryFor(deprecated), Set(ElasticIndexer.elasticIndex(view)))(pagination)
        case None =>
  ...
  def materialize(resource: Resource): EitherT[F, Rejection, ResourceV] =
    for {
  ...
  private final implicit class RefSyntax(ref: Ref) {
    ...
    def resolve(projectRef: ProjectRef): F[Option[Resource]] =
      projects.resolution(projectRef)(self).resolve(ref)
