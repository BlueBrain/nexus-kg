...
/**
  ...
  */
class ResolverIndexer[F[_]: Repo: Timer](
    projects: Projects[F],
  ...
  def apply(event: Event): F[Unit] = {
    ...
    val result: EitherT[F, Rejection, Boolean] = for {
      ...
      applied      <- EitherT.liftF(projects.applyResolver(projectRef, resolver))
    } yield applied
...
object ResolverIndexer {
  ...
  final def start(
    ...
    val indexer                     = new ResolverIndexer[Task](projects, resolution, retryConfig)
