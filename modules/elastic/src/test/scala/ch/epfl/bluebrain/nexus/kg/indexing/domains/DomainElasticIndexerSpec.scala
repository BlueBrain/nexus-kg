...
@DoNotDiscover
class DomainElasticIndexerSpec(esUri: Uri)
    extends TestKit(ActorSystem("DomainElasticIndexerSpec"))
    with IndexerFixture
    with WordSpecLike
    with Matchers
    with ScalaFutures
  ...
  private def getAll(index: String): Future[QueryResults[Json]] =
    client.search[Json](Json.obj("query" -> Json.obj("match_all" -> Json.obj())), indices = Set(index))(Pagination(0, 100))
  ...
  "A DomainElasticIndexer" should {
    ...
    "index a DomainCreated event" in {
      ...
      eventually {
        val rs = getAll(indexId).futureValue
