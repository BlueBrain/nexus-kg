...
object PatchQuery {
  ...
  def apply(json: Json, removeKeys: String*): Json = {
    val script = removeKeys.foldLeft("")((acc, field) => acc ++ s"""ctx._source.remove("$field");""")
    Json.obj(
  ...
  def inverse(json: Json, keepKeys: String*): Json = {
    val (prefix, sufix) = keepKeys.zipWithIndex.foldLeft("" -> "") {
      case ((p, s), (key, index)) =>
        val variable = s"var$index"
        (p + s"""Object $variable = ctx._source.get("$key");""") -> (s + s"""if($variable != null) ctx._source.put("$key", $variable);""")
    }
