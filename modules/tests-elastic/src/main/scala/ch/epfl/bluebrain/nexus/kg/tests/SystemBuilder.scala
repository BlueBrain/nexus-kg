...
object SystemBuilder {
  ...
  /**
    ...
    */
  final def initConfig(name: String, cassandraPort: Int, elasticPort: Int): ActorSystem = {
    val config = ConfigFactory
      .parseString(s"""
         |test.cassandra-port = $cassandraPort
         |test.elastic-port = $elasticPort
       """.stripMargin)
      .withFallback(ConfigFactory.parseResources("cluster-test.conf"))
      .withFallback(ConfigFactory.parseResources("cassandra-test.conf"))
      .withFallback(ConfigFactory.parseResources("elastic-test.conf"))
