...
/**
  ...
  */
class ContextRoutes(contexts: Contexts[Future], base: Uri)(implicit iamClient: IamClient[Future], ec: ExecutionContext)
  ...
  override protected def writeRoutes(implicit credentials: Option[OAuth2BearerToken]) =
    extractContextId { contextId =>
      pathEndOrSingleSlash {
        (put & entity(as[Json]) & authorizeResource(contextId, Write)) { json =>
          ...
          (delete & authorizeResource(contextId, Write)) {
        ...
        (path("config") & authorizeResource(contextId, Publish)) {
          (pathEndOrSingleSlash & patch & entity(as[ContextConfig])) { cfg =>
            parameter('rev.as[Long]) { rev =>
              if (cfg.published) {
...
object ContextRoutes {
  ...
  /**
    ...
    */
  final def apply(contexts: Contexts[Future], base: Uri)(implicit iamClient: IamClient[Future],
                                                         ec: ExecutionContext): ContextRoutes =
