...
class RelativeAttachmentLocationSpec extends WordSpecLike with Matchers with TryValues{
  ...
  abstract class Context{
    ...
    val rev = 3L
    val instanceId = InstanceId(SchemaId(DomainId(OrgId("organization"), "domain"), "schema", Version(1, 0, 0)), uuid.toString)
  }
  ...
  "A RelativeAttachmentLocation" should {
    ...
    "does not create a location when settings volume-path are incorrect" in new Context {
      ...
      fa.apply(instanceId, 3).failure.exception shouldEqual
        Unexpected(s"I/O error while trying to create directory for instance '$instanceId'. Error '/0'")
    }
  }
}
...
object RelativeAttachmentLocationSpec {
  ...
  val wrongConfig: Config = ConfigFactory.parseMap(
    Map("app.attachment.volume-path" -> "", "app.attachment.digest-algorithm" -> "SHA-256").asJava)
}
