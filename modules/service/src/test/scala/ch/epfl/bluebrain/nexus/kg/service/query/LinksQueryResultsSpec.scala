...
class LinksQueryResultsSpec extends WordSpecLike with Matchers {
  ...
  "A LinksQueryResults" should {
    ...
    "return the correct next links when pagination offset is 0" in {
      ...
      LinksQueryResults(resp, pagination, uri).links should contain theSameElementsAs List(Link("self", uri), Link("next", uri.withQuery(Query("size" -> "5", "other" ->"4","from" -> "5"))))
    ...
    "return the correct next and previous links when pagination offset is 5" in {
      ...
      LinksQueryResults(resp, pagination, uri).links should contain theSameElementsAs
        List(Link("self", uri), Link("previous", uri.withQuery(Query("size" -> "5", "from" -> "0"))), Link("next", uri.withQuery(Query("size" -> "5", "from" -> "10"))))
    }
    ...
    "return the correct previous and next links when offset is 2" in {
      ...
      val uri = Uri("http://localhost/v0/schemas/nexus/core")
      LinksQueryResults(resp, pagination, uri).links should contain theSameElementsAs
        List(Link("self", uri), Link("previous", uri.withQuery(Query("from" -> "0", "size" -> "2"))), Link("next", uri.withQuery(Query("from" -> "7", "size" -> "5"))))
    }
