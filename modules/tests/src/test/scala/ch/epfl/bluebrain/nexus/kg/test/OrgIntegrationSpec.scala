...
class OrgIntegrationSpec(apiUri: Uri, route: Route, vocab: Uri, sparqlClient: SparqlClient[Future], querySettings: BootstrapQuerySettings)(implicit
  ...
  import BootstrapIntegrationSpec._, orgsEncoder._
  ...
  "A OrganizationRoutes" when {
    ...
    "performing integration tests" should {
      ...
      "list organizations" in {
        eventually(timeout(Span(indexTimeout, Seconds)), interval(Span(1, Seconds))) {
          Get(s"/organizations") ~> route ~> check {
            ...
            val expectedResults = UnscoredQueryResults(orgs.length.toLong, orgs.map{UnscoredQueryResult(_)})
      ...
      "list organizations with pagination" in {
        ...
        val path = s"/organizations?from=${pagination.from}&size=${pagination.size}"

        eventually(timeout(Span(indexTimeout, Seconds)), interval(Span(1, Seconds))) {
          Get(path) ~> route ~> check {
            status shouldEqual StatusCodes.OK
            val expectedResults = UnscoredQueryResults(orgs.length.toLong, List(UnscoredQueryResult(orgs(1))))
            val expectedLinks = List(Link("self", s"$apiUri$path"), Link("previous", s"$apiUri$path".replace("from=1", "from=0")), Link("next", s"$apiUri$path".replace("from=1", "from=2")))
            responseAs[Json] shouldEqual LinksQueryResults(expectedResults, expectedLinks).asJson
          }
        }
      }
      ...
      "output the correct total even when the from query parameter is out of scope" in {
        ...
        Get(path) ~> route ~> check {
          status shouldEqual StatusCodes.OK
          val expectedResults = UnscoredQueryResults(orgs.length.toLong, List.empty[UnscoredQueryResult[OrgId]])
          val expectedLinks = List(Link("self", s"$apiUri$path"), Link("previous", s"$apiUri$path".replace("from=100", s"from=0")))
          responseAs[Json] shouldEqual LinksQueryResults(expectedResults, expectedLinks).asJson
        }
      }
      ...
      "list organization with full text search" in {
        ...
        Get(path) ~> route ~> check {
          status shouldEqual StatusCodes.OK
          val expectedResults = ScoredQueryResults(1L, 1F, List(ScoredQueryResult(1F, orgs.head)))
      ...
      "output the correct total in full text search even when the from query parameter is out of scope" in {
        ...
        Get(path) ~> route ~> check {
          status shouldEqual StatusCodes.OK
          val expectedResults = ScoredQueryResults(1L, 1F, List.empty[ScoredQueryResult[OrgId]])
      ...
      "list organizations with filter on 'path' organization " in {
        val uriFilter = URLEncoder.encode(s"""{"@context": {"rdf": "http://www.w3.org/1999/02/22-rdf-syntax-ns#"}, "filter": {"path": "${"organization".qualify}", "op": "eq", "value": "nexus"} } """, "UTF-8")
        val path = s"/organizations?filter=$uriFilter"
        Get(path) ~> route ~> check {
          status shouldEqual StatusCodes.OK
          val expectedResults = UnscoredQueryResults(1L, List(UnscoredQueryResult(orgs.head)))
          val expectedLinks = List(Link("self", s"$apiUri$path"))
          responseAs[Json] shouldEqual LinksQueryResults(expectedResults, expectedLinks).asJson
        }
      }
      ...
      "list organizations with deprecation" in {
        Get(s"/organizations?deprecated=false") ~> route ~> check {
          status shouldEqual StatusCodes.OK
          val expectedResults = UnscoredQueryResults(orgs.length.toLong, orgs.map{UnscoredQueryResult(_)})
          val expectedLinks = List(Link("self", s"$apiUri/organizations?deprecated=false"))
          responseAs[Json] shouldEqual LinksQueryResults(expectedResults, expectedLinks).asJson
        }
        Get(s"/organizations?deprecated=true") ~> route ~> check {
          status shouldEqual StatusCodes.OK
          val expectedResults = UnscoredQueryResults(0, List.empty[UnscoredQueryResult[OrgId]])
          val expectedLinks = List(Link("self", s"$apiUri/organizations?deprecated=true"))
          responseAs[Json] shouldEqual LinksQueryResults(expectedResults, expectedLinks).asJson
        }
      }
    }
