...
@DoNotDiscover
class DomainIntegrationSpec(apiUri: Uri, route: Route, vocab: Uri)(
    implicit
    as: ActorSystem,
    ...
    "performing integration tests" should {
      ...
      "list all domains" in {
        eventually(timeout(Span(indexTimeout, Seconds)),
          ...
          Get(s"/domains") ~> route ~> check {
            ...
            val expectedResults =
              UnscoredQueryResults(domains.size.toLong,
                                   domains.take(20).map(UnscoredQueryResult(_)))
            val expectedLinks =
              List(Link("self", s"$apiUri/domains"),
                   Link("next", s"$apiUri/domains?from=20&size=20"))
        ...
        eventually(timeout(Span(indexTimeout, Seconds)),
          ...
          Get(s"/domains/nexus") ~> route ~> check {
            ...
            val expectedResults = UnscoredQueryResults(
              5L,
              domains.filter(_.orgId.id == "nexus").map(UnscoredQueryResult(_)))
        ...
        eventually(timeout(Span(indexTimeout, Seconds)),
          ...
          Get(path) ~> route ~> check {
            ...
            val expectedLinks = List(Link("self", s"$apiUri$path"),
                                     Link("next", s"$apiUri$path&from=5"))
            responseAs[Json] shouldEqual LinksQueryResults(expectedResults,
                                                           expectedLinks).asJson
          }
        }
      ...
      "output the correct total even when the from query parameter is out of scope" in {
        ...
        Get(path) ~> route ~> check {
          status shouldEqual StatusCodes.OK
          val expectedResults =
            UnscoredQueryResults(randDomains.length.toLong,
                                 List.empty[UnscoredQueryResult[DomainId]])
          val expectedLinks = List(
            Link("self", s"$apiUri$path"),
            Link("previous",
                 s"$apiUri$path".replace("from=100",
                                         s"from=${randDomains.length - 5}")))
          responseAs[Json] shouldEqual LinksQueryResults(expectedResults,
                                                         expectedLinks).asJson
        }
      }
      ...
      "list domains on organization nexus and deprecated" in {
        eventually(timeout(Span(indexTimeout, Seconds)),
          ...
          Get(s"/domains/nexus?deprecated=false") ~> route ~> check {
            ...
            val expectedResults = UnscoredQueryResults(
              3L,
              domains.slice(2, 5).map(UnscoredQueryResult(_)))
            val expectedLinks =
              List(Link("self", s"$apiUri/domains/nexus?deprecated=false"))
            responseAs[Json] shouldEqual LinksQueryResults(expectedResults,
                                                           expectedLinks).asJson
          }
        }
