...
object QueryBuilder {
  ...
  object Field {
    ...
    /**
      ...
      */
    final case class Expr(val expr: String, override val variable: String) extends Field
    ...
    /**
      ...
      */
    final case class Var(override val variable: String) extends Field
    ...
    /**
      ...
      */
    implicit def applySelectExpr(tuple: (String, String)): Field = Expr(tuple._1, tuple._2)

    implicit val showField: Show[Field] =
      Show.show {
        case Expr(expr, v) => s"($expr AS ?$v)"
        case Var(v)        => s"?$v"
      }
  }
