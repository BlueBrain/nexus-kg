...
import ch.epfl.bluebrain.nexus.kg.core.instances.InstanceRejection.{AttachmentNotFound, IncorrectRevisionProvided, InstanceDoesNotExist, InstanceIsDeprecated, _}
...
class InstancesSpec extends WordSpecLike
  with Matchers
  with Inspectors
  with TryValues
  with Randomness
  ...
  abstract class Context {
    ...
    val domAgg = MemoryAggregate("dom")(Domains.initial, Domains.next, Domains.eval).toF[Try]
    val schemasAgg = MemoryAggregate("schema")(Schemas.initial, Schemas.next, Schemas.eval).toF[Try]
    ...
    val domRef = doms.create(DomainId(orgRef.id, genId()), "domain").success.value
    private val unpublished = schemas.create(SchemaId(domRef.id, genName(), genVersion()), schemaJson).success.value
    val schemaRef = schemas.publish(unpublished.id, unpublished.rev).success.value

    def createAttachmentRequest(id: InstanceId, rev: Long, source: String, filename: String, contentType: String): (Attachment.Info, Try[InstanceRef]) = {
      val expectedInfo = Attachment.Info(filename, contentType, Size(value = source.length.toLong), Digest("SHA-256", digestString(source)))
      expectedInfo -> instances.createAttachment(id, rev, expectedInfo.originalFileName, expectedInfo.contentType, source)
    }
  }
  ...
  "An Instances" should {
    ...
    "prevent update when deprecated" in new Context {
      ...
      instances.create(id, value).success.value shouldEqual InstanceRef(id, 1L)
      instances.deprecate(id, 1L).success.value shouldEqual InstanceRef(id, 2L)
      instances.update(id, 2L, genJson()).failure.exception shouldEqual CommandRejected(InstanceIsDeprecated)
    }
    ...
    "prevent update with incorrect rev" in new Context {
      ...
      val value = genJson()
      instances.create(id, value).success.value shouldEqual InstanceRef(id, 1L)
      instances.update(id, 2L, genJson()).failure.exception shouldEqual CommandRejected(IncorrectRevisionProvided)
    }
    ...
    "prevent creation if schema is locked" in new Context {
      ...
      instances.create(id, genJson()).failure.exception shouldEqual CommandRejected(SchemaRejection.SchemaIsDeprecated)
    }

    "prevent creation if schema is not published" in new Context {
      val unpublished = schemas.create(SchemaId(domRef.id, genName(), genVersion()), schemaJson).success.value
      val id = InstanceId(unpublished.id, genUUID())
      instances.create(id, genJson()).failure.exception shouldEqual CommandRejected(SchemaRejection.SchemaIsNotPublished)
    }
    ...
    "prevent creation if the json value does not conform to its schema" in new Context {
      ...
      instances.create(id, value).failure.exception match {
        case CommandRejected(ShapeConstraintViolations(vs)) => vs should not be 'empty
    ...
    "create a new instance attachment" in new Context {
      ...
      val (expectedInfo, response) = createAttachmentRequest(id, 1, source, "filename", "application/octet-stream")
    ...
    "create a new instance attachment and later update an instance should keep attachment" in new Context {
      ...
      instances.fetch(id).success.value shouldEqual Some(Instance(id, 3L, value2, Some(expectedInfo), deprecated = false))
    }
    ...
    "create several instance attachments, delete afterwards and fetch and older revisions" in new Context {
      ...
      val source2 = """{"one": "two"}"""
      val (expectedInfo2, response2) = createAttachmentRequest(id, 2, source2, "filename2", "application/octet-stream")
      ...
      instances.fetch(id, 2L).success.value shouldEqual Some(Instance(id, 2L, json, Some(expectedInfo), deprecated = false))
      instances.fetch(id, 3L).success.value shouldEqual Some(Instance(id, 3L, json, Some(expectedInfo2), deprecated = false))
    ...
    "prevent creation of attachment with incorrect rev" in new Context {
      ...
      instances.create(id, json).success.value shouldEqual InstanceRef(id, 1L)
      val source = "some_random_content"
      instances.createAttachment(id, 4, "filename", "application/octet-stream", source).failure.exception shouldEqual CommandRejected(IncorrectRevisionProvided)
    ...
    "prevent creation of attachment with instance that doesn't exist" in new Context {
      val id = InstanceId(schemaRef.id, genUUID())
      val source = "some_random_content"
      instances.createAttachment(id, 1, "filename", "application/octet-stream", source).failure.exception shouldEqual CommandRejected(InstanceDoesNotExist)
    }
    ...
    "prevent to remove an instance attachment when it does not exist" in new Context {
      val id = InstanceId(schemaRef.id, genUUID())
      instances.create(id, genJson()).success.value shouldEqual InstanceRef(id, 1L)
      instances.removeAttachment(id, 1L).failure.exception shouldEqual CommandRejected(AttachmentNotFound)
    }
  }
}
...
object InstancesSpec {
  ...
  class MockedInOutFileStream(implicit S: Show[InstanceId]) extends InOutFileStream[Try, String, String] {
    ...
    override def toSink(instanceId: InstanceId, rev: Long, filename: String, contentType: String, source: String): Try[Attachment.Meta] =
      Try {
        val relativeRoute = s"${Show[InstanceId].show(instanceId)}-$rev"
        val uri = new File(base, relativeRoute).toURI
        map.put(uri, source)
        Attachment.Meta(relativeRoute, Info(filename, contentType, Size(value = source.length.toLong), Digest("SHA-256", digestString(source))))
      }
  }
}