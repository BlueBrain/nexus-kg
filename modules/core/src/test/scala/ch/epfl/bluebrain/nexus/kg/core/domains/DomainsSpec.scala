...
class DomainsSpec
  ...
  "A Domains instance" should {
    ...
    "create a new domain" in {
      ...
      doms.fetch(id).success.value shouldEqual Some(
        Domain(id, 1L, deprecated = false, desc))
    }
    ...
    "prevent duplicate deprecations" in {
      ...
      doms.deprecate(id, 2L).failure.exception shouldEqual CommandRejected(
        DomainAlreadyDeprecated)
    }
    ...
    "prevent duplicate creations" in {
      ...
      val desc = genDesc()
      doms.create(id, desc).success.value shouldEqual DomainRef(id, 1L)
      doms.create(id, desc).failure.exception shouldEqual CommandRejected(
        DomainAlreadyExists)
    }
    ...
    "prevent creation with illegal id" in {
      ...
      forAll(
        List("",
             ...
             "123-",
             "ab",
             "abcdefakdlsjfalksjf93r83r8993798375")) { string =>
        val id = DomainId(orgId, string)
        doms.create(id, desc).failure.exception shouldEqual CommandRejected(
          InvalidDomainId(id.id))
      }
    }
