...
class SchemasSpec extends WordSpecLike
  with Matchers
  with Inspectors
  with TryValues
  with Randomness
  ...
  trait Context {
    ...
    val schemasAgg = MemoryAggregate("schemas")(Schemas.initial, Schemas.next, Schemas.eval).toF[Try]
  ...
  "A Schemas instance" should {
    ...
    "create a new schema" in new Context {
      ...
      schemas.fetch(id).success.value shouldEqual Some(Schema(id, 1L, schemaJson, deprecated = false, published = false))
    }

    "update a new schema" in new Context {
      val id = SchemaId(domRef.id, genName(), genVersion())
      schemas.create(id, schemaJson).success.value shouldEqual SchemaRef(id, 1L)
      schemas.update(id, 1L, optionalSchemaJson).success.value shouldEqual SchemaRef(id, 2L)
      schemas.fetch(id).success.value shouldEqual Some(Schema(id, 2L, optionalSchemaJson, deprecated = false, published = false))
    }
    ...
    "prevent update when deprecated" in new Context {
      ...
      schemas.create(id, schemaJson).success.value shouldEqual SchemaRef(id, 1L)
      schemas.deprecate(id, 1L).success.value shouldEqual SchemaRef(id, 2L)
      schemas.update(id, 2L, schemaJson).failure.exception shouldEqual CommandRejected(SchemaIsDeprecated)
    }
    ...
    "prevent update when published" in new Context {
      ...
      schemas.create(id, schemaJson).success.value shouldEqual SchemaRef(id, 1L)
      schemas.publish(id, 1L).success.value shouldEqual SchemaRef(id, 2L)
      schemas.update(id, 2L, schemaJson).failure.exception shouldEqual CommandRejected(CannotUpdatePublished)
    }

    "prevent update with incorrect rev" in new Context {
      val id = SchemaId(domRef.id, genName(), genVersion())
      schemas.create(id, schemaJson).success.value shouldEqual SchemaRef(id, 1L)
      schemas.update(id, 2L, schemaJson).failure.exception shouldEqual CommandRejected(IncorrectRevisionProvided)
    }
  ...
  "A Schemas" when {
    ...
    "domain is deprecated" should {
      ...
      "prevent updates" in new DomainDeprecatedContext {
        schemas.update(lockedId, 1L, genJson()).failure.exception shouldEqual CommandRejected(DomainIsDeprecated)
      }

      "prevent publishing" in new DomainDeprecatedContext {
        schemas.publish(lockedId, 1L).failure.exception shouldEqual CommandRejected(DomainIsDeprecated)
      }
      ...
      "allow deprecation" in new DomainDeprecatedContext {
        ...
        schemas.fetch(lockedId).success.value shouldEqual Some(Schema(lockedId, 2L, schemaJson, deprecated = true, published = false))
      }

      "prevent new schema creation" in new DomainDeprecatedContext {
        val id = SchemaId(domRef.id, genName(), genVersion())
        schemas.create(id, schemaJson).failure.exception shouldEqual CommandRejected(DomainIsDeprecated)
      }
    }
    ...
    "organization is deprecated" should {
      ...
      "prevent updates" in new OrgDeprecatedContext {
        schemas.update(lockedId, 1L, genJson()).failure.exception shouldEqual CommandRejected(OrgIsDeprecated)
      }

      "prevent publishing" in new OrgDeprecatedContext {
        schemas.publish(lockedId, 1L).failure.exception shouldEqual CommandRejected(OrgIsDeprecated)
      }
      ...
      "prevent new schema creation" in new OrgDeprecatedContext {
        val id = SchemaId(domRef.id, genName(), genVersion())
        schemas.create(id, schemaJson).failure.exception shouldEqual CommandRejected(OrgIsDeprecated)
      }
    }
  }
}
