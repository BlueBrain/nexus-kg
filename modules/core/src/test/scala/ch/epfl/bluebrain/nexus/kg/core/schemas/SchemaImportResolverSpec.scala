...
class SchemaImportResolverSpec
  ...
  private def schema(id: SchemaId,
                     ...
                     published: Boolean = true): Schema =
    uncheckedImportsSchema(id,
                           imports.map(i => s"$baseUri/schemas/${i.show}"),
                           published)
  ...
  "A SchemaImportResolver" should {
    ...
    "ignore incorrectly typed imports" in {
      val withUnknown =
        jsonImportsSchema(genSchemaId(),
                          List(Json.fromInt(12), Json.fromString("13")))
