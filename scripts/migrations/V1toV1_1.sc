...
@main def main(volume: String = "/opt/binaries",
  ...
  log.info(s"Dropping table $kgKeyspace.tag_views")
  session.execute(s"DROP TABLE IF EXISTS $kgKeyspace.tag_views")
  log.info(s"Dropping table $kgKeyspace.tag_scanning")
  session.execute(s"DROP TABLE IF EXISTS $kgKeyspace.tag_scanning")
  log.info(s"Dropping table $kgKeyspace.tag_write_progress")
  session.execute(s"DROP TABLE IF EXISTS $kgKeyspace.tag_write_progress")
  log.info(s"Dropping table $kgKeyspace.projections_failures")
  session.execute(s"DROP TABLE IF EXISTS $kgKeyspace.projections_failures")
